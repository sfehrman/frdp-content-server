...
The *package* process creates a deployable war file, in the current directory: `./target/content.war`: 
...
drwxrwxr-x 4 forgerock forgerock       51 Dec 19 16:00 content
-rw-rw-r-- 1 forgerock forgerock 12170899 Dec 19 16:00 content.war
...
This example deploys the `content.war` file to an Apache Tomcat 8.x environment.

## Deploy war file

Copy the `content.war` file to the `webapps` folder in the Tomcat server installation.  The running Tomcat server will automatically unpack the war file.
...
```
cd TOMCAT_INSTALLATION/webapps/content/WEB-INF/config
...
https://FQDN/tomcat/content/rest/content-server/content
```
...
```
> POST /tomcat/content/rest/content-server/content HTTP/1.1
> Content-type: application/json
> Content-Length: 41

< HTTP/1.1 201 
< Location: https://FQDN/tomcat/content/rest/content-server/content/e538bad7-8066-46f7-8e8f-ac36185aa2dc
```
...
```
> GET /tomcat/charlie/content/rest/content-server/content HTTP/1.1
...
curl -v -X GET \
-H "Accept: application/json" \
https://FQDN/tomcat/content/rest/content-server/content/e538bad7-8066-46f7-8e8f-ac36185aa2dc
```
...
```
> GET /tomcat/content/rest/content-server/content/e538bad7-8066-46f7-8e8f-ac36185aa2dc HTTP/1.1
> Accept: application/json
...
```
> PUT /tomcat/content/rest/content-server/content/e538bad7-8066-46f7-8e8f-ac36185aa2dc HTTP/1.1
> Content-type: application/json
> Content-Length: 102
...
```
> DELETE /tomcat/charlie/content/rest/content-server/content/e538bad7-8066-46f7-8e8f-ac36185aa2dc HTTP/1.1
